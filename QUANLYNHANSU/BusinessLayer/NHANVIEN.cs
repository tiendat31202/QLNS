    ...
    public class NHANVIEN
    {
        ...
        public List<NHANVIEN_DTO> getListFull()
        {
            ...
            foreach(var item in lstNV)
            {
                ...
                var bp = db.tb_BOPHAN.FirstOrDefault(x => x.IDBP == item.IDBP);
                nvDTO.TENBP = bp.TENBP;

                nvDTO.IDCV = item.IDCV;
                var cv = db.tb_CHUCVU.FirstOrDefault(x => x.IDCV == item.IDCV);
                nvDTO.TENCV = cv.TENCV;

                nvDTO.IDPB = item.IDPB;
                var pb = db.tb_PHONGBAN.FirstOrDefault(x => x.IDPB == item.IDPB);
                nvDTO.TENPB = pb.TENPB;

                nvDTO.IDTD = item.IDTD;
                var td = db.tb_TRINHDO.FirstOrDefault(x => x.IDTD == item.IDTD);
                nvDTO.TENTD = td.TENTD;

                nvDTO.IDDT = item.IDDT;
                var dt = db.tb_DANTOC.FirstOrDefault(x => x.ID == item.IDDT);
                nvDTO.TENDT = dt.TENDT;

                nvDTO.IDTG = item.IDTG;
                var tg = db.tb_TONGIAO.FirstOrDefault(x => x.ID == item.IDTG);
        ...
        public void Delete(int id)
        {
            try
            {
                var nv = db.tb_NHANVIEN.FirstOrDefault(x => x.MANV == id);
